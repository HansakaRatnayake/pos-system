...
const ItemUpdateForm = ({ onCreateFormClose, oldItem }) => {
  ...
  useEffect(() => {
    ...
    setItem({
      ...
      name: oldItem.name,
      price: oldItem.price,
      photo: photo || oldItem.photo, // Fallback to oldItem.photo if photo is null
      category: oldItem.category ? oldItem.category : {}, // Set entire category object
      code: oldItem.code
    })
  }, [oldItem])
...
console.log(item);
  ...
  const handleSubmit = (event) => {
    ...
    const obj = { ...item, code, photo: photoPreview ? photoPreview.startsWith('data') ? photoPreview.split(',')[1] : photoPreview : '' }
    console.log(obj);
  ...
  const handleCategoryChange = (event) => {
    const selectedCategory = categoryList.find(category => category.id === event.target.value);
    setItem(prevItem => ({
      ...prevItem,
            ...
            <Grid size={12} container>
              ...
              <Grid size={6}>
                <TextField
                  ...
                  fullWidth
                  size="medium"
                  value={item.category.id || ''}  // Reference the category id for the selected value
